...
--  when iam doing data cleaning i usually follow a few steps
...
-- so what iam going to do now is Creating a layoffs_staging2 table which will have row_num1 column and then deleting the rows where the row_num1 =2
...
-- yes it worked properly, so now i have this empty table and now i want to insert the informations
...
SELECT *
FROM layoffs_staging2;
-- now we actually not needed the row_num1 column so i can get rid of them by giving a drop query at the end.
...
SELECT *
FROM layoffs_staging2;
-- now i dont need the row_num1 colum so i need to get rid of them
-- lets fix it
